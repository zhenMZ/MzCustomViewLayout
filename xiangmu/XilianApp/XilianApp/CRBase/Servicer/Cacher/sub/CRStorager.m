...
static UIApplication *_CRSharedApplication() {
...
@implementation CRStoragerItem
@end

@implementation CRStorager
{
...
- (CRStoragerItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData
{
    int i = 0;
    ...
    CRStoragerItem *item = [CRStoragerItem new];
...
- (CRStoragerItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData
{
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData
{
    ...
    do
    {
        ...
        if (result == SQLITE_ROW)
        {
            CRStoragerItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init
{
    return [self initWithPath:@"" type:CRStoragerTypeFile];
}


- (instancetype)initWithPath:(NSString *)path type:(CRStoragerType)type
{
    self = [super init];
    ...
    if (self)
    {
        ...
        _trashQueue = dispatch_queue_create("Com.RogerAbyss.Cache.Disk.Trash", DISPATCH_QUEUE_SERIAL);
        ...
        if (![[NSFileManager defaultManager] createDirectoryAtPath:path
            ...
            ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&error]) {
                NSLog(@"CRStorager init error:%@", error);
                return nil;
            }
        ...
        if (![self _dbOpen] || ![self _dbInitialize])
        {
            ...
            {
                [self _dbClose];
                NSLog(@"CRStorager init error: fail to open sqlite db.");
                return nil;
            }
        }
...
+ (nullable instancetype)storagerInPath:(NSString *)path type:(CRStoragerType)type
{
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"CRStorager init error: invalid path: [%@].", path);
        return nil;
    }
...
- (void)dealloc
{
    UIBackgroundTaskIdentifier taskID = [_CRSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_CRSharedApplication() endBackgroundTask:taskID];
    }
}
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData
{
    ...
    if (_type == CRStoragerTypeFile && filename.length == 0)
    {
        return NO;
    }
...
- (BOOL)removeItemForKey:(NSString *)key
{
    ...
    switch (_type)
    {
        case CRStoragerTypeSQLite:
        {
            return [self _dbDeleteItemWithKey:key];
        }
            break;
        case CRStoragerTypeFile:
        case CRStoragerTypeMixed:
        {
            NSString *filename = [self _dbGetFilenameWithKey:key];
...
- (BOOL)removeItemsToFitSize:(int)maxSize
{
    ...
    do {
        ...
        for (CRStoragerItem *item in items)
        {
            if (total > maxSize)
            {
                if (item.filename)
...
- (CRStoragerItem *)getItemForKey:(NSString *)key
{
    if (key.length == 0) return nil;
    CRStoragerItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
...
- (CRStoragerItem *)getItemInfoForKey:(NSString *)key
{
    if (key.length == 0) return nil;
    
    CRStoragerItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];

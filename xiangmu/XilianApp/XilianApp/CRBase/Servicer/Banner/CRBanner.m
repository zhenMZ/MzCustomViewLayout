...
@interface CRBanner () <UICollectionViewDataSource, UICollectionViewDelegate>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UICollectionViewFlowLayout *flowLayout;

@property (nonatomic, strong) CRBannerFooter *footer;
...
- (void)updateSubviewsFrame
{
    ...
    // Footer宽度
    self.flowLayout.footerReferenceSize = CGSizeMake(64, self.frame.size.height);
...
- (void)fixDefaultPosition
{
    ...
    {
        // 总item数的中间
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:((self.itemCount * 10000) / 2) inSection:0]
...
- (void)autoScrollToNextItem
{
    ...
    if(nextItem >= (self.itemCount * 10000))
    {
        return;
    }
...
{
    CRBannerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"banner_item" forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)theCollectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)theIndexPath
{
    ...
    if(kind == UICollectionElementKindSectionFooter)
    {
        ...
        self.footer = (CRBannerFooter *)footer;
        
        // 配置footer的提示语
        if ([self.dataSource respondsToSelector:@selector(banner:titleForFooterWithState:)])
        {
            self.footer.idleTitle = [self.dataSource banner:self titleForFooterWithState:CRBannerFooterStateIdle];
            self.footer.triggerTitle = [self.dataSource banner:self titleForFooterWithState:CRBannerFooterStateTrigger];
        }
    }
    
    self.footer.hidden = !self.showFooter;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (footerDisplayOffset > 0)
    {
        ...
        if (footerDisplayOffset > 64)
        {
            ...
            self.footer.state = CRBannerFooterStateTrigger;
        }
        else
        {
            if (lastOffset < 0) return;
            self.footer.state = CRBannerFooterStateIdle;
        }
 ...
 *  数据源
 */
- (void)setDataSource:(id<CRBannerDataSource>)dataSource
{
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        // 注册cell
        [_collectionView registerClass:[CRBannerCell class] forCellWithReuseIdentifier:@"banner_item"];
        
        // 注册 \ 配置footer
        [_collectionView registerClass:[CRBannerFooter class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"banner_footer"];
        _collectionView.contentInset = UIEdgeInsetsMake(0, 0, 0, -64);
    }
 ...
 *  pageControl
 */
- (void)setPageControl:(CRPageControl *)pageControl
{
    // 移除旧oageControl
...
- (CRPageControl *)pageControl
{
    if (!_pageControl)
    {
        _pageControl = [[CRPageControl alloc] init];
        _pageControl.userInteractionEnabled = NO;
        _pageControl.autoresizingMask = UIViewAutoresizingNone;
    }

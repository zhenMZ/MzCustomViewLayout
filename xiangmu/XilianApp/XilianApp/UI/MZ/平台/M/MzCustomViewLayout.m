...
- (void)prepareLayout {
    ...
    for (NSUInteger i = 0; i < itemCounts; i++) {
        ...
        //x -- 尽可能用整数
        float origin_x = LayoutVertical ? shtIndex * [self columnWidth] + self.lineSpacing: [self.saveColumnHeightArray[shtIndex] integerValue];
        //y
        float origin_y = LayoutVertical ? [self.saveColumnHeightArray[shtIndex] integerValue] + self.rowSpacing: shtIndex * [self columnWidth];
        
        //width
        float size_width = 0.0;
        ...
        //height
        float size_height = 0.0;
        size_height = size.height;

        if (LayoutHorizontal) {
            
            float temp = size_width;

...
@implementation ZBrowserImageView

{
    __weak ZWaitingView *_waitingView;
    BOOL _didCheckSize;
...
- (void)layoutSubviews
{
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        if (!_scroll) {
            ...
            scroll.backgroundColor = MZPhotoBrowserBackgrounColor;
            _scroll = scroll;
...
{
    ZWaitingView *waiting = [[ZWaitingView alloc] init];
    waiting.bounds = CGRectMake(0, 0, 100, 100);
    waiting.mode = MZWaitingViewProgressMode;
    _waitingView = waiting;
    [self addSubview:waiting];
    
    
    __weak ZBrowserImageView *imageViewWeak = self;
...
- (void)prepareForImageViewScaling
{
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = MZPhotoBrowserBackgrounColor;
